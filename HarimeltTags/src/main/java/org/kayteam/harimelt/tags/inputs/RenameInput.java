...
public class RenameInput extends BookInput {
    ...
    public RenameInput(HarimeltTags harimeltTags, Player player) {
        ...
        setPages(configuration.getFileConfiguration().getStringList("editorInventory.bookInputText.rename"));
    }
    ...
    public boolean onFirm(Player player, String firm) {
        ...
        if (harimeltTags.getEditing().containsKey(uuid)) {
            ...
            if (!firm.contains(" ")) {
                tagManager.rename(name, firm);
                harimeltTags.getEditing().put(uuid, new String[] {"MENU", firm});
                TagEditInventory tagEditInventory = new TagEditInventory(harimeltTags);
                player.openInventory(tagEditInventory.getInventory(firm));
            } else {
                return false;
            }
        }
